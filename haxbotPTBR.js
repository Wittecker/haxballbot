	...
	const roomName = "FUTSAL x3 DO RALF";
	const botName = "Raffa Moreira Mano";
	const maxPlayers = 12;
	...
	function checkTime() {
		...
		if (Math.abs(scores.time - scores.timeLimit) <= 0.01 && scores.timeLimit != 0) {
			...
			goldenGoal = true;
			room.sendChat("⚽ Quem fizer é Viado !");
		}
	...
	function endGame(winner) { // handles the end of a game : no stopGame function inside
		...
		if (winner == Team.RED) {
			streak++;
			room.sendChat("🔴 Time vermelho Venceu " + scores.red + "-" + scores.blue + " ! Sequência de Vitória : " + streak + " 🏆");
		}
		else if (winner == Team.BLUE) {
			streak = 1;
			room.sendChat("🔵 Time azul venceu" + scores.blue + "-" + scores.red + " ! Sequência de Vitória : " + streak + " 🏆");
		}
		else {
			streak = 0;
	    ...
	    scores.red == 0 ? (scores.blue == 0 ? room.sendChat("🏆 " + GKList[0].name + " and " + GKList[1].name + " não tomou gol! ") : room.sendChat("🏆 " + GKList[1].name + " kept a CS ! ")) : scores.blue == 0 ? room.sendChat("🏆 " + GKList[0].name + " kept a CS ! ") : null;
		updateStats();
	...
	room.onPlayerJoin = function(player) {
		...
		room.sendChat("👋 Bem vindo " + player.name + ", seu pereba!", player.id);
		room.sendChat("Entre no nosso discord: https://discord.gg/ZFXn3BWy");
		if (localStorage.getItem(player.auth) != null) {
			if (JSON.parse(localStorage.getItem(player.auth))[Ss.RL] != "player") {
				room.setPlayerAdmin(player.id, true);
				room.sendChat((JSON.parse(localStorage.getItem(player.auth))[Ss.RL] == "master" ? "Master " : "Admin ") + player.name + " conectou!");
			}
		}
	}
		...
		if (["!help"].includes(message[0].toLowerCase())) {
			...
			if (players.length != 1 && player.team != Team.SPECTATORS) {
				if (player.team == Team.RED && streak > 0 && room.getScores() == null) {
					...
					room.sendChat("Você não pode ficar AFK", player.id);
					return false;
				}
			}
			...
			room.sendChat(player.name + (getAFK(player) ? " tá AFK" : " não tá mais AFK"));
			...
			if (message[1] == adminPassword) {
				...
				if (stats[Ss.RL] != "master") {
					stats[Ss.RL] = "master";
					room.sendChat(player.name + " ganhou ADM !");
			...
			if (player.admin) {
				if (message.length == 1) {
					slowMode = 3;
					room.sendChat("3 seconds slow mode enabled !");
				}
				...
				if (teamR.length <= teamB.length && player.id == teamR[0].id) { // we care if it's red turn && red cap talking
					if (["top", "auto"].includes(message[0].toLowerCase())) {
						...
						if (Number.parseInt(message[0]) > teamS.length || Number.parseInt(message[0]) < 1) {
							room.sendChat("Seu número é inválido!", player.id);
							return false;
						}
						else {
	...
	room.onTeamGoal = function(team) {
		...
		if (lastPlayersTouched[0] != null && lastPlayersTouched[0].team == team) {
			if (lastPlayersTouched[1] != null && lastPlayersTouched[1].team == team) {
				room.sendChat("⚽ " + getTime(scores) + " Gol do " + lastPlayersTouched[0].name + " ! Assist do " + lastPlayersTouched[1].name + ". Velocidade do chute : " + ballSpeed.toPrecision(4).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
				game.goals.push(new Goal(scores.time, team, lastPlayersTouched[0], lastPlayersTouched[1]));
			}
			else {
				room.sendChat("⚽ " + getTime(scores) + " Gol do " + lastPlayersTouched[0].name + " ! Velocidade do chute : " + ballSpeed.toPrecision(4).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
				game.goals.push(new Goal(scores.time, team, lastPlayersTouched[0], null));
			}
		}
		else {
			room.sendChat("😂 " + getTime(scores) + " Gol Contra do " + lastPlayersTouched[0].name + " ! Velocidade do chute : " + ballSpeed.toPrecision(4).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
